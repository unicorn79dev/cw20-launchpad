...
export default {
  ...
  computed: {
    depositAmountHuman () {
      var amount = Utils.convertMicroDenomToDenom(this.item.amount, 6)
      var decimals = 18
      if (amount > 1) {
        decimals = 2
    ...
    percentOfTotal () {
      var locked = Number(this.item.amount) / Math.pow(10, 6)
      var total = Number(this.totalSupply) / Math.pow(10, 6)

      console.log("locked amount = ", locked)
